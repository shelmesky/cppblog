...
file_server::file_server(cppcms::service &srv,bool async) : application(srv), async_(async)
{
	...
	if(settings().find("file_server.alias").type()==cppcms::json::is_array) {
		cppcms::json::array const &alias = settings().find("file_server.alias").array();
		for(unsigned i=0;i<alias.size();i++) {
			std::string url = alias[i].get<std::string>("url");
			if(url.size() < 2 || url[0]!='/') {
				throw std::runtime_error("Invalid alias URL: " + url);
			}
			if(url[url.size()-1]=='/')
			...
			if(!canonical(input_path,canon_path)) {
				throw std::runtime_error("Invalid alias path: " + input_path);
			}
...
void file_server::list_dir(std::string const &url,std::string const &path)
{
	...
	out << "<html><head><title>Directory Listing</title></head>\n"
		"<body><h1>Index of " << cppcms::util::escape(url) << "</h1>\n";
	...
	while(d.next()) {
		...
		out << "<tr>";
		out << "<td><code><a href='" 
			<< cppcms::util::urlencode(d.name()) << add << "'>" << cppcms::util::escape(d.name()) << add << "</a></code></td>";
